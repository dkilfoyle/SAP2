<template>
  <div style="background-color:rgb(36,30,30); height:60px; padding-left:5px">
    <canvas ref="mycanvas" height="60" width="40"></canvas>
  </div>
</template>

<script>
// Forked from http://www.3quarks.com
...
export default {
  ...
  methods: {
    ...
    draw: function() {
      var display = this.$refs.mycanvas
      if (display) {
    ...
    drawDigit: function(context, xPos, mask, c) {
      switch (mask) {
        case '#':
          ...
          if (this.segmentCount === 16) {
            ...
            switch (this.cornerType) {
              case this.SegmentDisplay.SymmetricCorner:
                context.moveTo(x + s + d, y + s)
                context.lineTo(x + this.segmentWidth + d, y)
                break
              case this.SegmentDisplay.SquaredCorner:
          ...
          context.fillStyle = this.getSegmentColor(
            c,
            '013456789abdhnouy',
            '01346789abdghjmnoqsuw@',
            '%'
          )
          context.beginPath()
    ...
    drawPoint: function(context, x1, y1, size) {
      var x2 = x1 + size
      var y2 = y1 + size
      var d = size / 4.0
    ...
    getSegmentColor: function(c, charSet7, charSet14, charSet16) {
      if (c === '#') {
        return this.colorOn
      } else {
