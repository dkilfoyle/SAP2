<template>
  <div :id="id" ref="digits" :style="style" :class="{ resize: resize }" tabindex="0">
...
export default {
  ...
  computed: {
    divStyle: function() {
      return 'display: inline-block; height: 100%; ' + 'width: ' + 100 / this.colors.length + '%;'
    },
    colors: function() {
    ...
    style: function() {
      var str = 'float: right; display: inline-block; padding: 0.5em'
  ...
  methods: {
     ...
     */
    toPrecision(val) {
      return this.precision !== undefined ? parseFloat(val.toFixed(this.precision)) : val
    }
  }
}
</script>
