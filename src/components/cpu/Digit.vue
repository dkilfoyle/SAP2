<template>
  <div>
    <svg viewBox="0 0 60 80" :style="styleComputed" :value="value" :dp="dp" :slant="slant">
      <circle :style="dpStyle" cx="55" cy="75" r="5"></circle>
      <polyline
      ...
      ></polyline>
      <path id="backcursor" :class="back" :style="'stroke: ' + colorOn" d="M 55 0 L 55 80"></path>
      <path id="frontcursor" :class="front" :style="'stroke: ' + colorOn" d="M 0 0 L 0 80"></path>
    </svg>
  </div>
</template>
<script>
  ...
  computed: {
    styleComputed: function() {
      ...
      if (parseFloat(this.slant) != 0) {
        style = style + 'transform: skewX(' + -parseFloat(this.slant) + 'deg); '
      }
      return style
    },
    dpStyle: function() {
